...
theorem decomp_a1_sum : ∀ n k : Nat, decomp_a1_aux n k + decomp_a2_aux n k = n := by
  ...
  | _ n ih =>
    ...
    | succ n' =>
      ...
      rw [eq_a1, eq_a2]
      -- Now we have: a1 + (c*3^k + (a2 + r*3^k)) = n' + 1
      -- where a1 = decomp_a1_aux ((n'+1)/3) (k+1), a2 = decomp_a2_aux ((n'+1)/3) (k+1)
      -- 
      -- Reassociate to: (a1 + a2) + (c*3^k + r*3^k)
      rw [Nat.add_assoc (decomp_a1_aux ((n' + 1) / 3) (k + 1)) (decomp_c ((n' + 1) % 3) * 3^k) (decomp_a2_aux ((n' + 1) / 3) (k + 1) + decomp_r ((n' + 1) % 3) * 3^k)]
      -- = a1 + (c*3^k + (a2 + r*3^k))
      -- Now we want: a1 + ((a2 + r*3^k) + c*3^k) (commuting and reassociating)
      rw [Nat.add_comm (decomp_c ((n' + 1) % 3) * 3^k) (decomp_a2_aux ((n' + 1) / 3) (k + 1) + decomp_r ((n' + 1) % 3) * 3^k)]
      -- = a1 + ((a2 + r*3^k) + c*3^k)
      rw [Nat.add_assoc (decomp_a2_aux ((n' + 1) / 3) (k + 1) + decomp_r ((n' + 1) % 3) * 3^k) (decomp_c ((n' + 1) % 3) * 3^k) (decomp_a1_aux ((n' + 1) / 3) (k + 1))]
      -- Wait, this goes the wrong way. Let me think.
      -- Actually I want: a1 + (c*3^k + (a2 + r*3^k)) = (a1 + a2) + (c*3^k + r*3^k)
      -- That requires swapping c*3^k with (a2 + r*3^k) and then reassociating.
      sorry
